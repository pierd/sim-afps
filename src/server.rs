use ambient_api::{prelude::*, core::{player::components::is_player, transform::components::rotation}};
use packages::{afps_schema::{components::*, messages::*}, this::{components::is_simulated, messages::StopSim}};
...
pub fn main() {
    StartGame::subscribe(|ctx, msg| {
        if let Some(id) = ctx.client_entity_id() {
            eprintln!("start game for player {}, the player will be simulated", msg.name);
            entity::add_component(id, is_simulated(), true);
            // just replicate the fpsui logic to start the game for player
            entity::add_component(id, player_name(), msg.name);
        }
    });
    StopSim::subscribe(|_ctx, msg| {
        entity::remove_component(msg.player_id, is_simulated());
    });
    process_fps_movement()
}

const INIT_JUMP_VSPEED: f32 = 0.10;
const FALLING_VSPEED: f32 = 0.4;

fn process_fps_movement() {
